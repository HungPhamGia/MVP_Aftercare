...
def _system_prompt(questions: list[str], patient: dict) -> str:
    ...
    return (
        ...
        "MỘT câu, theo thứ tự. CHỈ được hỏi đúng các câu có trong BỘ CÂU HỎI; TUYỆT ĐỐI KHÔNG "
        "tự nghĩ ra câu hỏi thăm khám nào khác (đau, ăn, ngủ, vết mổ, đại tiện...) nếu nó KHÔNG "
        "nằm trong bộ. Hỏi xong câu CUỐI CÙNG của bộ → chuyển NGAY sang bước kết thúc (luật 9), "
        "không hỏi lan man. Với câu trả lời tự do của bệnh nhân, được hỏi thêm tối đa 1 câu "
        "ngắn để làm rõ CHÍNH câu đó (mức độ, thời gian, vị trí...) rồi quay lại bộ câu hỏi. Riêng khi bệnh "
